...
class LabelAttention(nn.Module):
    def __init__(self, hidden_dim: int, num_labels: int) -> None:
        ...
        self.label_emb = nn.Parameter(torch.rand(num_labels, hidden_dim).uniform_(-0.1, 0.1))

    def forward(self, context: Tensor) -> Tuple[Tensor, Tensor]:
        con = torch.tanh(self.fc(context))  # [batch_size, seq_len, hidden]
        label_emb = torch.transpose(self.label_emb, 0, 1)
        score = torch.matmul(con, label_emb)
        # [batch_size, seq_len, num_labels]  = [batch_size, seq_len, hidden] * [hidden, num_labels]
...
class AdditiveAttention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        query = query.unsqueeze(1).repeat(1,128,1)
        score = self.score_proj(torch.tanh(self.key_proj(key) + self.query_proj(query) + self.bias))
        attn = F.softmax(score, dim=-1)
        # attn = torch.diag_embed(attn)
        # context = torch.bmm(attn, value)
        context = torch.mul(attn, value)
        return context, attn
